      ...
      foreach($orders_to_send as $order){
        ...
        if($order_data->getShippingAmount()>0){
            $a_sp = array(
                  ...
                  'qty'=> 1,
                  'price' => $order_data->getShippingTaxAmount()+$order_data->getPaymentSurchargeAmount(),
                  'price_before_discount' => $order_data->getShippingTaxAmount()+$order_data->getPaymentSurchargeAmount(),
                  'name' => 'Koszty wysyłki',
                  'id_store' => $this->subiekt_api_warehouse_id,
            );
